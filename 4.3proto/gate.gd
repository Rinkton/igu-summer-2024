...
func set_up_symmetric_object():
	if get_parent().name == "above":
		...
		$CollisionShape2D.position = get_node("symmetry_link").obj.get_node("CollisionShape2D").position
		$CollisionShape2D.scale = get_node("symmetry_link").obj.get_node("CollisionShape2D").scale
		$CollisionShape2D.shape.size = get_node("symmetry_link").obj.get_node("CollisionShape2D").shape.size
